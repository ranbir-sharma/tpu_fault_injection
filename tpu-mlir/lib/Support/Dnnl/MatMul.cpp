...
void MatMul::run() {
  ...
  errs() << "THIS IS EXECUTING " << "\n";


  // auto dstPtr = static_cast<float*>(dst_mem.get_data_handle());

  // Get the memory descriptor of dst_mem
  // auto dst_desc = dst_mem.get_desc();

  // // Retrieve the dimensions of dst_mem
  // memory::dims dst_dims = dst_desc.get_dims();

  // The batch_size would be the first dimension, which is dst_dims[0]
  // int batch_size = dst_dims[0];

  // Extract the dimensions
  // int batch_size = dst_dims[0];  // batch * batch_low
  // int M = dst_dims[1];           // number of rows
  // int N = dst_dims[2];           // number of columns

  // Iterate through each element in the output matrix
  // for (int b = 0; b < batch_size; ++b) {
  //   for (int i = 0; i < M; ++i) {
  //     for (int j = 0; j < N; ++j) {
  //       int index = b * M * N + i * N + j;  // Calculate the 1D index

  //       // Access the element
  //       //float value = dstPtr[index];
  //       float value = origin_output[index];

        


  //       // Optionally modify the value (e.g., fault injection)
  //       // dstPtr[index] = value * 1.1f;  // Example modification
  //       origin_output[index] = 100.0;  // Example modification
  //       errs() << "Value Going back " << value * 1.1f << "\n";  
  //     }
  //   }
  // }
