...
struct xbyak_gemm_t : public jit_generator {
    ...
    xbyak_gemm_t(char isTransA, char isTransB, float beta, 
            bool FaultInjection,
            bool tmp_traceing,
            int *tmp_numberofMul,
            int *tmp_randomInjection,
            int tmp_faultBit,
            bool hasBias = false,
        ...
        , STACK_K_CAPACITY((STACK_CAPACITY - 256) / (SIZE * UNROLL_M)) {
            ...
            tracing = tmp_traceing;
            numberofMul = tmp_numberofMul;
            randomInjection = tmp_randomInjection;
            faultBit = tmp_faultBit;
    ...
    void fma(bool useFma, const Ymm &reg0, const Ymm &reg1, const Ymm &reg2,
        ...
        if(tracing) {
            *numberofMul = *numberofMul + 1;    
            if(injectFault) {
                //   localResult = true;
                if((*randomInjection) == (*numberofMul)) {
                      std::cout << "Fault injected inside FMA" << std::endl;
                    localResult = true;
                }
            }
        ...
        if(localResult) {
            const Ymm tmpReg(1 << faultBit);
            // std::cout << "This is okay " << std::endl;
            std::cout << "Doing Fault injected inside FMA" << std::endl;

            // mov(tmpReg, 1);
            vxorps(reg2, reg2, tmpReg); // XOR reg2 with the value in xmm0
            // vsubps(reg2, reg2, reg2);
            // pop(reg0);
            // std::cout << "This is okay3 " << std::endl;
    ...
    void generate() override ATTRIBUTE_OPTIMIZE {
        ...
        std::cout << "Starting the kernel 1 " << "\n";

        if(tracing) {
        cout << "injectFault is " << injectFault << "\n";
        cout << "tracing is " << tracing << "\n";
        cout << "tracing is " << faultBit << "\n";
        if(numberofMul != nullptr) {
        cout << "numberofMul is " << *numberofMul << "\n";
        }
        else { cout << "numberofMul is nullptr" << "\n";
        }
        if(randomInjection != nullptr) {
        cout << "randomInjection is " << *randomInjection << "\n";
        } else {
            cout << "randomInjection is nullptr" << "\n";
        }
...
xbyak_gemm_t *get_xbyak_gemm(
        bool isTransA, bool isTransB, float beta, bool hasBias, bool tracing, bool injectFault, 
            int *tmp_numberofMul, int *tmp_randomInjection, int tmp_faultBit) {
    auto beta_idx = [](float beta) {
        return (beta == 0.0) ? 0 : (beta == 1.0 ? 1 : 2);
    };

    std::cout << "Its here now " << endl;

    // Kernel table [isTransA][isTransB][hasBias][beta (0, 1, other)]
    static maybe_unique_ptr<xbyak_gemm_t> kernel_table[2][2][2][3];
    static maybe_unique_ptr<xbyak_gemm_t> kernel_table1[2][2][2][3];
    static maybe_unique_ptr<xbyak_gemm_t> kernel_table2[2][2][2][3];
    static std::once_flag initialized;
    static std::once_flag initialized1;
    static std::once_flag initialized2;
    static std::atomic<dnnl_status_t> st(dnnl_success);
    static std::atomic<dnnl_status_t> st1(dnnl_success);
    static std::atomic<dnnl_status_t> st2(dnnl_success);
                ...
                for (bool hasBias : {false, true})
                    for (float beta : {0.0f, 1.0f, 2.0f}) {
                        ...
                        kern.reset(new xbyak_gemm_t(
                                isTransA, isTransB, beta, false, false, nullptr, nullptr, 0, hasBias));
            ...
            for (bool isTransB : {false, true})
                for (bool hasBias : {false, true})
                    for (float beta : {0.0f, 1.0f, 2.0f}) {
                        // nocopy sgemm with bias for beta != 0.0 is not supported
                        if (hasBias && beta != 0.0) continue;
                        auto &kern = kernel_table1[isTransA][isTransB][hasBias]
                                                 [beta_idx(beta)];

                        kern.reset(new xbyak_gemm_t(
                                isTransA, isTransB, beta, true, true, tmp_numberofMul, tmp_randomInjection, tmp_faultBit, hasBias));
                        if (kern->create_kernel() != dnnl_success) {
                            st1 = dnnl_runtime_error;
                            return;
                        }
                    }
    });

            return (st1 == dnnl_success)
            ? kernel_table1[isTransA][isTransB][hasBias][beta_idx(beta)].get()
            : nullptr;
    }
    else if(tracing) {


    std::call_once(initialized2, [&] {
        for (bool isTransA : {false, true})
            for (bool isTransB : {false, true})
                for (bool hasBias : {false, true})
                    for (float beta : {0.0f, 1.0f, 2.0f}) {
                        // nocopy sgemm with bias for beta != 0.0 is not supported
                        if (hasBias && beta != 0.0) continue;
                        auto &kern = kernel_table2[isTransA][isTransB][hasBias]
                                                 [beta_idx(beta)];

                        kern.reset(new xbyak_gemm_t(
                                isTransA, isTransB, beta, false, true, tmp_numberofMul, tmp_randomInjection, tmp_faultBit, hasBias));
                        if (kern->create_kernel() != dnnl_success) {
                            st2 = dnnl_runtime_error;
                            return;
                        }
                    }
    });

            return (st2 == dnnl_success)
            ? kernel_table2[isTransA][isTransB][hasBias][beta_idx(beta)].get()
            : nullptr;
    }
...
dnnl_status_t sgemm_nocopy_driver(const char *transa, const char *transb,
        ...
        dim_t ldc, const float *bias, bool tracing, bool injectFault, 
            int *tmp_numberofMul,
            int *tmp_randomInjection, int tmp_faultBit) {
    ...
    auto ker_bn = get_xbyak_gemm(isTransA, isTransB, *beta, hasBias, tracing, injectFault, tmp_numberofMul, tmp_randomInjection, tmp_faultBit);
    auto ker_b1 = get_xbyak_gemm(isTransA, isTransB, 1.0, false, tracing, injectFault, tmp_numberofMul, tmp_randomInjection, tmp_faultBit);
    auto ker_b0 = get_xbyak_gemm(isTransA, isTransB, 0.0, false, tracing, injectFault, tmp_numberofMul, tmp_randomInjection, tmp_faultBit);
    ...
    for (Bk = 0; Bk < k; Bk += sizeK) {
        ...
        for (Bm = 0; Bm < m; Bm += sizeM) {
            ...
            for (Bn = 0; Bn < n; Bn += sizeN) {
                ...
                if (Bk == 0) {
                    std::cout << "Going into the kernel " << endl;
   ...
   bool helperfunction(int *tmp_faultBit) {
    bool result1 = false;
  ...
  if (!std::getline(file, first_line)) {
    std::cerr << "The file is empty or does not have a valid first line." << std::endl;
    return result1;
  }

  // Read the second line
  if (!std::getline(file, second_line)) {
    std::cerr << "The file does not have a second line." << std::endl;
    return result1;
  }

  // Read the third line
  if (!std::getline(file, third_line)) {
    std::cerr << "The file does not have a third line." << std::endl;
    return result1;
  }
  ...
  // Convert the second line to a number and increment it
  int number2;
  int number1;
  int number3;
  try {
    number1 = std::stoi(first_line);
    number2 = std::stoi(second_line); // Convert string to integer
    number3 = std::stoi(third_line);
  } catch (const std::exception &e) {
    std::cerr << "Failed to convert second line to a number: " << e.what() << std::endl;
    return result1;
  }
  number2++; // Increment the number
  *tmp_faultBit = number3;
    if(number1 == number2) {
    std::cout << "Got the conv layer we are targetting" << "\n";
    result1 = true;
    }
  ...
  std::ofstream outfile(file_name, std::ios::trunc); // Open in truncate mode
  if (!outfile.is_open()) {
  ...
  if (!remaining_content.empty()) {
      outfile << remaining_content; // Write the remaining content back
  }
  outfile.close();

  std::cout << "Updated second line to: " << number2 << "\n";
...
dnnl_status_t jit_avx_gemm_f32(int nthrs, const char *transa,
    ...
    bool result = helperfunction(&tmp_faultBit);

    std::cout << "Bit Fault is " << tmp_faultBit << std::endl;
    ...
    if (nthr_to_use == 1) {
        auto status = sgemm_nocopy_driver(transa, transb, m, n, k, p_alpha, A,
                lda, B, ldb, p_beta, C, ldc, bias, false, false, nullptr, nullptr, tmp_faultBit);
        return status;
    }
    ...
    parallel(nthr_spawn, [&](int ithr, int nthr) {
        ...
        if (ithr < nthr_m * nthr_n * nthr_k) {
            ...
            if ((myM > 0) && (myN > 0)) {
                ...
                int numberofMul = 0;
                int tmp_randomInjection = 0;

                st_thr = sgemm_nocopy_driver(transa, transb, myM,
                        myN, myK, p_alpha, myA, lda, myB, ldb, &myBeta, myC, ld,
                        myBias, tracing, injectFault, &numberofMul, &tmp_randomInjection, tmp_faultBit);
                ...
                // st_thr = sgemm_nocopy_driver(transa, transb, myM,
                //         myN, myK, p_alpha, myA, lda, myB, ldb, &myBeta, myC, ld,
                //         myBias, tracing, injectFault, &numberofMul, &tmp_randomInjection);

                // tracing = true;
                // injectFault = true;

                // First lets see the total number of mul instr
                std::cout << "Total number of mul is " << numberofMul << "\n";
                ...
                std::uniform_int_distribution<int> dist(1, numberofMul);

                // Generate the random conv layer number
                tmp_randomInjection = dist(gen);
                std::cout << "Fault injection in " << tmp_randomInjection << "\n";
                ...
                st_thr = sgemm_nocopy_driver(transa, transb, myM,
                 ...
                 st_thr = sgemm_nocopy_driver(transa, transb, myM,
                        myN, myK, p_alpha, myA, lda, myB, ldb, &myBeta, myC, ld,
                        myBias, false, false, nullptr, nullptr, tmp_faultBit);
                }
